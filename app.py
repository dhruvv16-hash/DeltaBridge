...
def webhook():
    ...
    try:
        ...
        # 5. Process signal across all active accounts
        for account in active_accounts:
            ...
            try:
                ...
                if action in ["close_long", "close_short"]:
                    logger.info(f"Processing close request for {symbol} on account '{account.name}'...")
                    pos = client.get_position(product_id)
                    if not pos:
                        logger.info(f"No open position found for {symbol} on account '{account.name}'.")
                    ...
                    if pos_size == 0:
                        logger.info(f"Position size is 0 for {symbol} on account '{account.name}'.")
                    ...
                    if action == "close_long" and not is_long:
                        logger.warning(f"Received close_long alert but position is not LONG (size: {raw_size}) on account '{account.name}'. Ignoring.")
                    ...
                    if action == "close_short" and not is_short:
                        logger.warning(f"Received close_short alert but position is not SHORT (size: {raw_size}) on account '{account.name}'. Ignoring.")
                    ...
                    if res.get("success"):
                        logger.info(f"Successfully closed position for {symbol} on account '{account.name}'. Order ID: {res.get('result', {}).get('id')}")
                        account_result.update({"success": True, "response": res})
                    else:
                        logger.error(f"Failed to close position on account '{account.name}': {res}")
                ...
                # Check for buy/sell entries
                    ...
                    if pos:
                        ...
                        if is_reversal and abs(pos_size) > 0:
                            logger.info(f"Reversal detected! Closing opposing position of size {abs(pos_size)} first on account '{account.name}'...")
                            ...
                            if close_res.get("success"):
                                logger.info(f"Opposing position closed on account '{account.name}'. Sleeping for 1.5 seconds for margin release...")
                                time.sleep(1.5)
                            else:
                                logger.error(f"Failed to close opposing position on account '{account.name}' during reversal: {close_res}")
                    ...
                    try:
                        ...
                        logger.error(f"Invalid price value received for {symbol} on account '{account.name}': '{price_str}'")
                    ...
                    balance, asset = client.get_available_balance()
                    if balance <= 0:
                        msg = f"Available balance is 0 on account '{account.name}'."
                        logger.warning(msg)
                        account_result.update({"success": False, "message": msg})
                        results.append(account_result)
                        continue

                    # Calculate max lots based on account settings
                    buying_power = balance * account.leverage * (account.balance_buffer_pct / 100.0)
                    ...
                    logger.info(f"Dynamic Sizing details for account '{account.name}': Balance = {balance} {asset}, Leverage = {account.leverage}x, "
                                f"Buffer = {account.balance_buffer_pct}%, Lot USD Value = {lot_value_usd:.4f}, Calculated Qty = {qty_lots} Lots")

                    if qty_lots <= 0:
                        msg = f"Insufficient balance ({balance} {asset}) for leverage {account.leverage}x to open even 1 lot on account '{account.name}'."
                        logger.warning(msg)
                        account_result.update({"success": False, "message": msg})
                        results.append(account_result)
                        continue
                    ...
                    if res.get("success"):
                        logger.info(f"Successfully entered position for {symbol} on account '{account.name}'. Order ID: {res.get('result', {}).get('id')}")
                        account_result.update({"success": True, "response": res})
                    else:
                        logger.error(f"Failed to place order on account '{account.name}': {res}")
                        account_result.update({"success": False, "message": "Delta API Order Placement Failed", "details": res})

            except Exception as acc_e:
                logger.exception(f"Exception processing webhook for account '{account.name}': {acc_e}")
